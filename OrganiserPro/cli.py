...
@click.option(
    "--recursive/--no-recursive", default=True, help="Search subdirectories recursively"
)
@click.option(
    "--delete", is_flag=True, help="Delete duplicate files (keeping the oldest)"
)
@click.option(
    "--move-to",
...
def dedupe(
    ...
    try:
        find_duplicates_cli(
            directory=directory, recursive=recursive, delete=delete, move_to=move_to
        )
    except Exception as e:
        console.print(f"[red]Error: {e}")
        sys.exit(1)
